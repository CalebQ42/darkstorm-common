...
class Frame extends StatefulWidget {
  ...
  static FrameState of(BuildContext context) =>
      context.findAncestorStateOfType<FrameState>()!;
}
...
class FrameState extends State<Frame> {
  ...
  void updateItems() {
    ...
    if (widget.floatingItem != null) {
      (widget.floatingItem!.key! as GlobalKey<FloatingNavState>)
          .currentState
          ?.setState(() {});
    }
  }
  ...
  EdgeInsets get contentMargin => hidden
      ? EdgeInsets.zero
      : vertical
          ? const EdgeInsets.only(top: 50)
          : const EdgeInsets.only(left: 50);
  Matrix4? get transform => !expanded
      ? Matrix4.translationValues(0, 0, 0)
      : vertical
          ? Matrix4.translationValues(0, _verticalTranslation, 0)
  ...
  EdgeInsets get topItemMargin => hidden
      ? EdgeInsets.zero
      : vertical
  ...
  Widget build(BuildContext context) {
    ...
    var navHeight = 50 +
        (widget.navItems.length * 50) +
        (widget.bottomNavItems.length * 50);
    if (!vertical && widget.floatingItem != null) navHeight += 50;
    var navExpand = 0.0;
    var verticalPadding = media.padding.top +
        media.padding.bottom +
        media.viewInsets.top +
        media.viewInsets.bottom;
    var horizontalPadding = media.padding.left +
        media.padding.right +
    ...
    var navItems = [
      topNav(ti),
      AnimatedContainer(duration: ti.transitionDuration, height: navExpand),
      ...widget.navItems,
      AnimatedContainer(duration: ti.transitionDuration, height: navExpand),
          ...
          children: [
            ...
            Stack(children: [
              AnimatedContainer(
                ...
                height:
                    (vertical ? (media.size.height / 2) : media.size.height) -
                        verticalPadding,
                child: ListView(
                  controller: scrol,
                  physics: (vertical && expanded) || !vertical
  ...
  Widget topNav(TopResources ti) {
    Widget inner = AnimatedContainer(
        duration: ti.globalDuration,
        margin: () {
          EdgeInsets margin = ti.frame.vertical
              ? EdgeInsets.zero
        ...
        child: AnimatedAlign(
            duration: ti.globalDuration,
            alignment:
                ti.frame.expanded ? Alignment.center : Alignment.centerLeft,
          ...
          if (vertical)
            AnimatedSwitcher(
                ...
                transitionBuilder: (child, anim) => SizeTransition(
                    ...
                    axisAlignment: -1.0,
                    child: child),
                child: !ti.frame.expanded ? widget.floatingItem : null)
        ],
      );
    }
